"""
🤖 GÖREV YAPSAM BOTU v8.0 - SADE VE ETKİLİ
...
def add_ref(referrer_id, referred_id):
    with get_db() as conn:
        ...
        cursor.execute('''INSERT INTO refs (referrer_id, referred_id, earned)
...
Referans: {user['referrals']}

Hemen görev yap para kazan!"""
...
Katıldıktan sonra "✅ KATILDIM" butonuna bas.""",
    ...
    # Hoşgeldin mesajı
    welcome_msg = f"""<b>Hoş geldin {first_name}!</b>
...
def create_task_menu(user_id, message_id):
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton("🤖 BOT GÖREV VER (2.50 ₺)", callback_data="create_bot"),
        types.InlineKeyboardButton("📢 KANAL GÖREV VER (1.50 ₺)", callback_data="create_kanal"),
        types.InlineKeyboardButton("👥 GRUP GÖREV VER (1.00 ₺)", callback_data="create_grup")
    )
    markup.add(types.InlineKeyboardButton("🏠 MENÜ", callback_data="back_menu"))
...
def process_broadcast(message):
    ...
    for user in users:
        try:
            ...
            sent += 1
            time.sleep(0.05)  # Rate limit için
        except:
            failed += 1
